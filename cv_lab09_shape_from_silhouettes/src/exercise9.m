...
if computeVisualHull
    ...
    for n = 1:numCameras
        %iterate through points within the colume
        for i = 1:volumeX
            for j = 1:volumeY
                for k = 1:volumeZ
                    %Compute projection into image
                    cordinate_2d = Ps{n} * T * [i; j; k; 1];
                    %Normalize homogenous coordinates
                    cordinate_2d = cordinate_2d / cordinate_2d(end);
                    
                    %Check if projection is within image plane
                    x_coordinate = round(cordinate_2d(2));
                    y_coordinate = round(cordinate_2d(1));
                    %check for point beeing within image
                    if x_coordinate > 0 && x_coordinate < size(sils{n},2) && y_coordinate > 0 && y_coordinate < size(sils{n},1)
                        %check if it lies within the silhouette
                        if sils{n}(x_coordinate,y_coordinate) == 1
