...
% Legi Number: 14-921-969
% Name: Manuel Galliker
...
intermiediate_measurement = door1 + door2 + door3;
measurement_sum = sum(intermiediate_measurement);
measurement = intermiediate_measurement/measurement_sum;
...
motion = uk * simpar.T;
motion_disturbance = random('norm',0,simpar.odometry_stdev,[1, simpar.numberOfParticles]);

% TODO: Change this value for the correct one
predict_particles = mod(priorbelief_particles + motion + motion_disturbance, simpar.domain);
...
door1 = pdf('Normal', zk, abs(predict_particles - simpar.door_locations(1)), simpar.door_stdev);
door2 = pdf('Normal', zk, abs(predict_particles - simpar.door_locations(2)), simpar.door_stdev);
door3 = pdf('Normal', zk, abs(predict_particles - simpar.door_locations(3)), simpar.door_stdev);

intermiediate_particle_weights = door1 + door2 + door3;
if(zk==0)
   intermiediate_particle_weights = 1 / simpar.numberOfParticles * ones(1, simpar.numberOfParticles);
end
particle_weights_sum = sum(intermiediate_particle_weights);
particle_weights = intermiediate_particle_weights/particle_weights_sum;
...
% TODO: Change this value for the correct one
Particles = zeros(size(predict_particles));
b_cum = cumsum(particle_weights);
random_numbers = rand(1,simpar.numberOfParticles);

for (i = 1:simpar.numberOfParticles)
    Particles(i) = predict_particles(find(random_numbers(i) <= b_cum,1));
end


updated_belief_particles = Particles;
